...
import { toRow, FILTER_TABS, STATUS_OPTIONS } from "./lib/reviews.helpers";

export default function ReviewsPage() {
  const t = useT();
  const { data, isLoading, isError, refetch } = useReviews();
  const updateStatus = useUpdateReviewStatus();
  // URL state: /reviews?status=pending — shareable & survives refresh.
      ...
      {isLoading ? (
        ...
        <>
          <ReviewsTable
            ...
            onStatus={(id, st) => updateStatus.mutate({ id, st: st as ReviewStatusKey })}
          />
          <Pagination
            page={pg.page}
